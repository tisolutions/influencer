...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your TatianaArias account.<br>
        ...
        &mdash; TatianaArias Team
      `
      return sendMail({ toEmail: email, subject: 'TatianaArias - Password Reset', content })
    })
